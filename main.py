...
from data_processing import load_dataset_from_csv
...
def predict_stock(coin_name):
    dataset, new_data = load_dataset_from_csv(coin_name)
...
app.layout = html.Div([
    ...
    dcc.Tabs(id="tabs1", children=[
        render_tab(btc_train.index, btc_valid["Close"], btc_valid.index, btc_valid["Predictions"],
                   label=tickers["btc"]),
        render_tab(eth_train.index, eth_valid["Close"], eth_valid.index, eth_valid["Predictions"],
                   label=tickers["eth"]),
        render_tab(ada_train.index, ada_valid["Close"], ada_valid.index, ada_valid["Predictions"],
                   label=tickers["ada"]),
    ]),
